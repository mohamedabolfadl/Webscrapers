...
while pc<=npages :
    ...
    for offer in all_offers:
        ...
        if rev:
            ...
            ret = soup.find("li",class_="list-icon-ele")
            tmp = ret.text
            tmp = tmp.encode('utf-8')
            tmp = tmp[2:len(tmp)-1]
            meals.append(tmp)
    ...
    i=1
    all_stars = soup_packtpage.find_all("span",class_=r)
    #print all_stars
...
try:
    ...
    for ji in range(len(prices_hotels)):
        writer.writerow(( hotel_ids[ji] ,prices_hotels[ji] , airports[ji] , dep_dates[ji] , ret_dates[ji] , durations[ji] , recommendation_values[ji] ,nummber_recs[ji] , strs[ji],meals[ji], hotel_names[ji] ))
finally:
    csvFile.close()
...
for ci in range(0,len(durations)):
    ...
    if meals[ci][0]=='n': # Only sleep
        mark_code=mark_code+'o'
    if meals[ci][0]=='F': # Breakfast
        mark_code=mark_code+'s'
    if meals[ci][0]=='H': # Half board
        mark_code=mark_code+'^'
    if meals[ci][0]=='V': # Full board
        mark_code=mark_code+'d'
    if meals[ci][0]=='A': # Full board
        mark_code=mark_code+'*'
    ...
    plt.text(0.5+nummber_recs[ci],-5+prices_hotels[ci], str(ci+1)+"|"+str(recommendation_values[ci]), fontdict=font_sel)
