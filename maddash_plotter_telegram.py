...
def sendNews(t):
    ...
    while i<len(inf):
        ...
        if IsInt(stra(f).replace(".","")) and IsInt(stra(p).replace(".","")) and IsInt(stra(a).replace(".","")):
            ...
            plt.bar(pls, forecast_change ,width,  edgecolor =col,alpha=0.8,fill=False,linewidth=2, align='center')
            plt.bar(pls, previous_change, width, edgecolor =cunfound,alpha=0.3,fill=False,linewidth=1, align='center')
            ax.text(pls-0.25, 1, stra(c[4])+'\n'+stra(c[5])+'\n'+stra(c[6]), fontsize=12,color=col)
            pls = pls+1
    ...
    plt.xticks(cnt, xlb, rotation='vertical', fontsize = 7)
    plt.margins(0.2)
